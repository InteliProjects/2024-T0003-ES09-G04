...
namespace Web.Api.Track.co.Service
{
    public class WidgetService
    {
        ...
        public async Task<string> GenerateHTML(Widget widget)
        {
            string question = widget.Question;

            Console.WriteLine("Generating HTML for widget with ID " + widget.Id + " and question: " + question);

            var js =
                $@"<script>document.getElementById('widgetForm').addEventListener('submit', function (e) {{ e.preventDefault(); const formData = new FormData(e.target); const widgetId = {widget.Id}; const data = {{ widgetId, answer: formData.get('answer') || '', rating: parseInt(formData.get('rating'), 10) }}; fetch(`http://localhost:5244/widgets/{widget.Id}/nps`, {{ method: 'POST', headers: {{ 'Content-Type': 'application/json', }}, body: JSON.stringify(data), }}).then(response => response.json()).then(data => {{ console.log('Success:', data); alert('Form submitted successfully!'); }}).catch((error) => {{ console.error('Error:', error); alert('An error occurred while submitting the form.'); }}); }});</script>";
            var css =
            ...
            // var storeOnS3 = new StoreOnS3();         
            // await storeOnS3.Store(widget.Id, js, css);

            var html =
                // $@"<link rel=""stylesheet"" href=""https://track-co-api.s3.amazonaws.com/widget-{widget.Id}-scripts.js""><form id='widgetForm' class='widget-container'><p class='widget-title'>Welcome to the widget</p><div><label class='widget-label'>{question}</label></div><div><label class='widget-label'>Answer:</label><input class='widget-input' name='answer' placeholder='Your answer'optional></div><div><label class='widget-label' for='rating'>Rating (1-10):</label><select class='widget-select' name='rating'required><option value=''>Choose a rating</option><option value='1'>1</option><option value='2'>2</option><option value='3'>3</option><option value='4'>4</option><option value='5'>5</option><option value='6'>6</option><option value='7'>7</option><option value='8'>8</option><option value='9'>9</option><option value='10'>10</option></select></div><button type='submit' class='widget-button'>Send</button></form><script src=""https://track-co-api.s3.amazonaws.com/widget-{widget.Id}-scripts.js""></script>";
                $@"<link rel=""stylesheet"" href=""https://track-co-api.s3.amazonaws.com/widget-{widget.Id}-styles.css""><form id='widgetForm' class='widget-container'><p class='widget-title'>Welcome to the widget</p><div><label class='widget-label'>{question}</label></div><div><label class='widget-label'>Answer:</label><input class='widget-input' name='answer' placeholder='Your answer'optional></div><div><label class='widget-label' for='rating'>Rating (1-10):</label><select class='widget-select' name='rating'required><option value=''>Choose a rating</option><option value='1'>1</option><option value='2'>2</option><option value='3'>3</option><option value='4'>4</option><option value='5'>5</option><option value='6'>6</option><option value='7'>7</option><option value='8'>8</option><option value='9'>9</option><option value='10'>10</option></select></div><button type='submit' class='widget-button'>Send</button></form><script src=""https://track-co-api.s3.amazonaws.com/widget-{widget.Id}-scripts.js""></script>";
        ...
        public async Task<Widget> CreateWidget(WidgetCreateDto widgetDto)
        {
            try
            {
                widgetDto.Validate();
                ...
                {
                    Id = _context.Widgets.Any() ? _context.Widgets.Max(w => w.Id) + 1 : 1,
                ...
                var html = await GenerateHTML(widget);
                widget.Html = html;
        ...
        public async Task<Nps> CreateNps(int widgetId, NPSCreateDto npsDto)
        {
        ...
        public async Task<Widget> UpdateWidget(int id, WidgetUpdateDto widgetUpdateDto)
        {
            try
            {
                ...
                var newHtml = await GenerateHTML(widget);
                widget.Html = newHtml;
